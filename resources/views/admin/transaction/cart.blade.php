...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-8rem)]">
    ...
    <div class="lg:col-span-2 bg-white rounded-lg shadow-sm p-4 overflow-y-auto">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3" id="products-grid">
            ...
            <div class="product-card bg-white border rounded-lg p-3 cursor-pointer relative overflow-hidden hover:shadow-md transition-shadow duration-300">
                ...
                <div class="mb-2">
                    <h3 class="text-xs font-medium text-gray-800 mb-1 line-clamp-2 leading-tight">{{ $product->name }}</h3>
                    <p class="text-xs text-blue-600 font-semibold mb-1">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
                ...
                <button onclick="addToCart('{{ $product->id }}', '{{ htmlspecialchars($product->name, ENT_QUOTES, 'UTF-8') }}', {{ is_numeric($product->price) ? $product->price : 0 }}, {{ is_numeric($product->stock) ? $product->stock : 0 }})" 
                ...
                @else
                <div class="absolute bottom-2 right-2 w-7 h-7 bg-gray-400 text-white rounded-full flex items-center justify-center cursor-not-allowed">
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 overflow-y-auto">
        ...
        <div class="border-b pb-4 mb-4">
            ...
            <div class="mb-4">
                ...
                <select id="santri_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <option value="">-- Pilih Santri (opsional untuk non-santri) --</option>
                    @foreach($santris as $santri)
                        <option value="{{ $santri->id }}">{{ $santri->name }} - Saldo: Rp {{ number_format($santri->saldo, 0, ',', '.') }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="payment_type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
...
function showFlashMessage(type, title, message, icon = null) {
    ...
    flashDiv.className = `flash-message relative ${config.bgColor} ${config.textColor} p-4 rounded-lg shadow-lg border-l-4 border-white`;
...
function addToCart(id, name, price, stock) {
    console.log('addToCart called:', { id, name, price, stock });
    ...
    if (existingItem) {
        if (existingItem.qty < stock) {
            ...
            showFlashMessage('warning', 'Stok Terbatas!', 'Jumlah produk dalam keranjang sudah mencapai batas stok yang tersedia.', 'fas fa-exclamation-triangle');
            return;
        }
    } else {
...
function clearCart() {
    showConfirmDialog(
        ...
        () => {
            ...
            updateCartDisplay();
            syncCartWithBackend();
            showFlashMessage('info', 'Keranjang Dikosongkan', 'Semua produk telah dihapus dari keranjang belanja.', 'fas fa-shopping-cart');
        }
    );
}
...
function updateQuantity(id, change) {
    ...
    if (item) {
        ...
        if (newQty <= 0) {
            ...
            showFlashMessage('warning', 'Stok Terbatas!', `Stok ${item.name} hanya tersedia ${item.stock} unit.`, 'fas fa-exclamation-triangle');
        }
    }
}
...
function processPayment() {
    if (cart.length === 0) {
        showFlashMessage('warning', 'Keranjang Kosong!', 'Silakan tambahkan produk ke keranjang terlebih dahulu.', 'fas fa-shopping-cart');
        return;
    }
    ...
    if (paymentType === 'saldo' && !santriId) {
        showFlashMessage('warning', 'Pilih Santri!', 'Silakan pilih santri terlebih dahulu untuk pembayaran menggunakan saldo.', 'fas fa-user');
        return;
    }

    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
    if (!csrfToken) {
        showFlashMessage('error', 'Kesalahan Konfigurasi', 'Token CSRF tidak ditemukan. Silakan muat ulang halaman.', 'fas fa-bug');
        return;
    }
    ...
    .then(data => {
        ...
        if (data.success) {
            const santriName = santriId ? document.getElementById('santri_id').selectedOptions[0].text.split(' - ')[0] : 'Non-Santri';
